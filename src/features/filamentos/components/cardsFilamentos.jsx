...
/**
 * SUB-COMPONENTE: BARRA SEGMENTADA (Padronizada)
 */
        ...
        `}>
                ...
                <div className="flex-1 p-6 flex flex-col justify-between min-w-0">
                    ...
                    <div className="flex justify-between items-start gap-2">
                        <div className="min-w-0 flex-1">
                            <h3 className="text-lg font-black uppercase tracking-tight truncate leading-none text-zinc-100 group-hover:text-sky-400 transition-colors">
                                {item?.nome || "Sem Nome"}
                            </h3>
                        ...
                        <div className={`shrink-0 px-2 py-1 rounded-md border text-[8px] font-black uppercase tracking-widest ${stats.ehCritico ? 'bg-rose-500/20 border-rose-500/40 text-rose-400 animate-pulse' : 'bg-zinc-900 border-zinc-800 text-zinc-600'
                            }`}>
            ...
            <div className="grid grid-cols-[1fr_50px_50px] h-11 bg-zinc-950/60 border-t border-zinc-800/50">
                <button
                    ...
                    className="flex items-center justify-center gap-2.5 text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 hover:text-sky-400 hover:bg-sky-500/[0.03] transition-all group/btn"
                >
...
export const FilamentRow = memo(({ item, onEdit, onDelete, onConsume }) => {
    ...
    return (
        <div className={`
            grid grid-cols-[60px_1fr_repeat(3,50px)] h-12 bg-zinc-900/40 backdrop-blur-sm rounded-xl overflow-hidden transition-all duration-300
            border ${ehCritico ? 'border-rose-500/30 bg-rose-500/[0.02]' : 'border-zinc-800/60 hover:border-zinc-700/80'} items-center group
        `}>
            {/* ÍCONE LATERAL ROW */}
            ...
            <div className="flex items-center px-6 justify-between min-w-0 h-full">
                <div className="flex items-center gap-4 min-w-0">
                    <h3 className="text-[11px] font-black uppercase tracking-wider truncate text-zinc-200 group-hover:text-sky-400 transition-colors">
                        {item?.nome || "Sem identificação"}
                    </h3>
                    <span className="text-[8px] font-black px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-500 uppercase tracking-tighter">
                        {item?.material}
                    </span>
                </div>
            ...
            <button
                onClick={() => onEdit(item)}
                title="Editar"
                className="flex items-center justify-center h-full border-l border-zinc-800/50 text-zinc-600 hover:text-zinc-100 hover:bg-zinc-800/40 transition-all"
            >
                <Edit2 size={14} />
            </button>
            <button
