...
export default function ModalRegistrarFalha({ aberto, aoFechar, aoSalvar }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await axios.post('/api/failures', form);
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/90 backdrop-blur-sm modal-backdrop">
            ...
            <div className="relative bg-zinc-950 border border-orange-500/30 rounded-2xl w-full max-w-md shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">

                <header className="p-6 border-b border-zinc-800/50 bg-zinc-900/10 flex justify-between items-start">
                    <div>
                ...
                <footer className="p-6 pt-2 bg-zinc-900/10 flex flex-col gap-3">
                    <FormFeedback
                        type={feedback.type}
                    ...
                    <button
                        ...
                        className="w-full py-4 rounded-xl bg-orange-600 hover:bg-orange-500 text-white text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-orange-900/20"
                    >
