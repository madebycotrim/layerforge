...
const PrinterCard = memo(({ printer, onEdit, onDelete, onResetMaint, onToggleStatus }) => {
    ...
    return (
        <div className={`
            group relative bg-zinc-900/40 backdrop-blur-sm rounded-2xl overflow-hidden 
            transition-all duration-300 hover-lift
            border ${stats.ehCritico
                ? 'border-rose-500/40 bg-rose-500/[0.03] shadow-[0_0_20px_rgba(244,63,94,0.05)]'
                : 'border-zinc-800/60 hover:border-zinc-700/80 hover:shadow-xl shadow-sm'}
        `}>
                    ...
                    <button
                        ...
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 
                            border active:scale-90 hover:scale-105 ${printer.status === 'printing'
                                ...
                                : 'bg-zinc-900/50 border-zinc-800 text-zinc-600 hover:text-zinc-400 hover:border-zinc-700 hover:bg-zinc-800/50'
                            }`}
                    >
            ...
            <div className="grid grid-cols-[1fr_50px_50px] h-11 bg-zinc-950/60 border-t border-zinc-800/50">
                <button onClick={() => onResetMaint?.(printer)}
                    className="flex items-center justify-center gap-2.5 text-[9px] font-black uppercase tracking-[0.2em] 
                        text-zinc-500 hover:text-zinc-100 hover:bg-zinc-800/30 transition-all duration-300 group/btn"
                >
