...
import { X, Zap, Timer, DollarSign, Tag, Binary, Terminal, Cpu, Activity, Loader2, AlertCircle } from "lucide-react";
...
export default function PrinterModal({ aberto, aoFechar, aoSalvar, dadosIniciais }) {
    ...
    const handleTentativaFechar = useCallback(() => {
        if (isSaving) return;
        if (isDirty) {
            if (window.confirm("As alterações não salvas serão perdidas. Quer mesmo sair?")) {
                aoFechar();
            }
        } else {
            aoFechar();
    ...
    const isValid = formulario.nome.trim() !== "" && formulario.marca.trim() !== "" && formulario.modelo.trim() !== "";
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/90 backdrop-blur-sm animate-in fade-in duration-300">
            ...
            <div className={`relative bg-zinc-950 border border-zinc-800/80 rounded-[2rem] w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] transition-all duration-300 ${isSaving ? 'opacity-90 grayscale-[0.3]' : ''}`}>
                ...
                <div className="flex-1 flex flex-col bg-zinc-950">
                    <header className="px-10 py-6 border-b border-zinc-800/50 flex justify-between items-center bg-zinc-900/10">
                        ...
                        <button disabled={isSaving} onClick={handleTentativaFechar} className="p-2 rounded-full hover:bg-zinc-900 text-zinc-500 transition-all disabled:opacity-20">
                            <X size={20} />
                        </button>
                    </header>
                    ...
                    <footer className="p-8 border-t border-zinc-800/50 bg-zinc-900/10 flex gap-4">
                        ...
                        <button disabled={isSaving} onClick={handleTentativaFechar} className="flex-1 py-3 text-[11px] font-bold uppercase text-zinc-400 border border-zinc-800 rounded-xl hover:bg-zinc-800 hover:text-zinc-100 transition-all disabled:opacity-20">
                            Cancelar
                        </button>
                        <button
                            disabled={!isValid || isSaving}
                            onClick={handleSalvarInterno}
                            className={`flex-[2] py-3 text-[11px] font-bold uppercase rounded-xl flex items-center justify-center gap-3 transition-all ${isValid && !isSaving ? "bg-zinc-100 text-zinc-950 hover:bg-white active:scale-95 shadow-xl" : "bg-zinc-900 text-zinc-600 cursor-not-allowed"}`}
                        >
