...
const HistoryStatCard = ({ title, value, icon: Icon, colorClass, secondaryLabel, secondaryValue }) => (
    <div className="h-[135px] p-6 rounded-2xl bg-[#0a0a0b] border border-zinc-900 flex items-center justify-between group transition-all hover:border-sky-500/20 shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
        ...
        <div className="text-right flex flex-col justify-between h-full py-1">
            ...
            <div className="flex items-center gap-2 justify-end opacity-20 group-hover:opacity-100 transition-opacity">
                <span className="text-[8px] font-black text-zinc-600">HISTÓRICO</span>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <span className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest">
                            ...
                            {item.timestamp} • Cód. {item.id?.slice(0,6).toUpperCase()}
                        </span>
                    </div>
                </div>

                <div className="text-right flex flex-col items-end">
                    <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest mb-1">VALOR SUGERIDO</span>
            ...
            <div className="flex items-end justify-between border-t border-white/5 pt-5 mt-auto">
                <div className="flex gap-4">
                    <div className="flex flex-col gap-1">
                        <span className="text-[7px] font-black text-zinc-700 uppercase tracking-widest">Impressora</span>
                        <div className="flex items-center gap-2 px-2.5 py-1 bg-black rounded-lg border border-zinc-800">
                            <Zap size={10} className="text-amber-500" />
                            <span className="text-[9px] font-bold text-zinc-400 uppercase">{e.impressoraNome || "Perfil Manual"}</span>
                        </div>
                    </div>
                    <div className="flex flex-col gap-1">
                        <span className="text-[7px] font-black text-zinc-700 uppercase tracking-widest">Filamento</span>
                        <div className="flex items-center gap-2 px-2.5 py-1 bg-black rounded-lg border border-zinc-800">
                ...
                <div className="flex items-center gap-4">
                    <div className="w-32 flex flex-col items-end">
                        <div className="flex justify-between w-full mb-1">
                             <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest">MARGEM</span>
                    ...
                    <button onClick={onDelete} title="Remover orçamento" className="w-9 h-9 flex items-center justify-center rounded-xl bg-rose-500/5 border border-rose-500/10 text-rose-500/40 hover:text-rose-500 hover:bg-rose-500/10 hover:border-rose-500/30 transition-all">
                        <Trash2 size={16} />
                    </button>
                </div>
            </div>
        ...
        <div className="flex h-screen w-full bg-[#050505] text-zinc-100 font-sans overflow-hidden">
            ...
            <main className="flex-1 flex flex-col relative transition-all duration-300 ease-in-out" style={{ marginLeft: `${larguraSidebar}px` }}>
                ...
                <header className="h-20 px-8 flex items-center justify-between z-40 relative border-b border-white/5 bg-[#050505]/80 backdrop-blur-xl">
                    <span>
                        <h1 className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-600 mb-1">Arquivo de Pedidos</h1>
                        <span className="text-2xl font-black tracking-tight text-white uppercase tracking-tighter">Histórico de Orçamentos</span>
                    </span>

                    <div className="flex items-center gap-6">
                        <div className="flex bg-zinc-900/30 border border-zinc-800/60 p-1.5 rounded-2xl backdrop-blur-md">
                            <button onClick={() => setViewMode('grid')} className={`p-2 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-zinc-800 text-sky-400 shadow-lg' : 'text-zinc-600 hover:text-zinc-400'}`} title="Grade"><LayoutGrid size={16} /></button>
                            <button onClick={() => setViewMode('list')} className={`p-2 rounded-xl transition-all ${viewMode === 'list' ? 'bg-zinc-800 text-sky-400 shadow-lg' : 'text-zinc-600 hover:text-zinc-400'}`} title="Lista"><List size={16} /></button>
                        </div>
                        ...
                        <div className="relative group">
                            ...
                            <input className="w-72 bg-zinc-900/20 border border-zinc-800/50 rounded-2xl py-2.5 pl-11 pr-4 text-[11px] text-zinc-300 outline-none font-mono focus:border-sky-500/30 focus:bg-zinc-900/40 transition-all placeholder:text-zinc-700" placeholder="PROCURAR PROJETO OU MÁQUINA..." value={busca} onChange={e => setBusca(e.target.value)} />
                        </div>

                        <button onClick={() => window.confirm("Quer mesmo apagar todo o seu histórico de projetos?") && (clearHistory(), carregarDados())} className="h-11 px-6 border border-rose-500/20 hover:bg-rose-500/10 text-rose-500 rounded-2xl text-[10px] font-black uppercase flex items-center gap-3 transition-all active:scale-95">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-10 relative z-10">
                    <div className="max-w-[1650px] mx-auto space-y-16">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                            <HistoryStatCard title="Faturamento Total" value={formatCurrency(metrics.totalVenda)} icon={DollarSign} colorClass="text-emerald-500" secondaryLabel="Valor de Venda" secondaryValue="Soma de todos os projetos" />
                            <HistoryStatCard title="Projetos Salvos" value={historico.length.toString().padStart(2, '0')} icon={FileText} colorClass="text-sky-500" secondaryLabel="Total de Arquivos" secondaryValue="Salvo no navegador" />
                            <HistoryStatCard title="Lucro Médio" value={`${metrics.margemMedia}%`} icon={TrendingUp} colorClass="text-purple-500" secondaryLabel="Margem média" secondaryValue="Desempenho da oficina" />
                            <HistoryStatCard title="Tempo de Bico" value={`${metrics.totalTempo}h`} icon={Clock} colorClass="text-amber-500" secondaryLabel="Tempo de Máquina" secondaryValue="Horas totais estimadas" />
                        </div>
                            ...
                            {metrics.filtrados.length > 0 ? (
                                metrics.filtrados.map((item) => (
                                    <QuoteProtocolCard 
                                        key={item.id} 
                                        item={item} 
                                        onDelete={() => window.confirm("Deseja excluir este orçamento da sua lista?") && (removeHistoryEntry(item.id), carregarDados())} 
                                    />
                                ))
                            ) : (
                                <div className="col-span-full py-40 flex flex-col items-center justify-center border border-dashed border-zinc-900 rounded-[3rem] bg-zinc-950/40 backdrop-blur-sm">
                                    ...
                                    <p className="text-zinc-600 text-[12px] font-black uppercase tracking-[0.4em]">Nenhum registro por aqui</p>
                                </div>
                            )}
                        </div>
                    </div>
