...
export async function handleFilaments(method, url, idFromPath, db, userId, request) {
    ...
    if (['POST', 'PUT', 'PATCH'].includes(method)) {
        ...
        if (method === 'PATCH') {
            const field = f.peso_atual !== undefined ? 'peso_atual' : 'favorito';
            const value = field === 'peso_atual' ? toNum(f.peso_atual) : (f.favorito ? 1 : 0);
            await db.prepare(`UPDATE filaments SET ${field} = ? WHERE id = ? AND user_id = ?`).bind(value, id, userId).run();
        ...
        await db.prepare(`INSERT INTO filaments (id, user_id, nome, marca, material, cor_hex, peso_total, peso_atual, preco, data_abertura, favorito) 
            ...
            .bind(id, userId, f.nome, f.marca || "", f.material || "", f.cor_hex || "#000000", toNum(f.peso_total), toNum(f.peso_atual), toNum(f.preco), f.data_abertura, f.favorito ? 1 : 0).run();
