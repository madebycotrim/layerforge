...
export async function handleFilaments({ request, db, userId, pathArray, url }) {
    ...
    try {
        ...
        if (method === 'DELETE') {
            ...
            if (!id) return sendJSON({ error: "ID do filamento necessário." }, 400);

            await db.prepare("DELETE FROM filaments WHERE id = ? AND user_id = ?").bind(id, userId).run();
            return sendJSON({ success: true, message: "Filamento removido com sucesso." });
        }
        ...
        if (['POST', 'PUT', 'PATCH'].includes(method)) {
            ...
            if (method === 'PATCH') {
                ...
                return sendJSON({ success: true, message: "Estoque atualizado." });
            }
        ...
        return sendJSON({ error: "Erro ao processar filamentos", details: error.message }, 500);
    }
}