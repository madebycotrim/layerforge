...
export async function handleFailures({ request, db, userId }) {
    ...
    try {
        ...
        if (method === 'POST') {
            ...
            if (f.filamentId && f.filamentId !== 'manual') {
                ...
                if (filamento) {
                    const novoPeso = Math.max(0, filamento.peso_atual - toNum(f.weightWasted));
                    await db.prepare("UPDATE filaments SET peso_atual = ? WHERE id = ?").bind(novoPeso, f.filamentId).run();
                }
            }

            return sendJSON({ success: true, message: "Falha registrada e estoque atualizado." });
        }

    } catch (error) {
        return sendJSON({ error: "Erro ao processar falhas", details: error.message }, 500);
    }
}
