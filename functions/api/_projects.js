...
export async function handleApproveBudget(db, userId, request) {
    ...
    const p = await request.json();
    const project = await db.prepare("SELECT data FROM projects WHERE id = ? AND user_id = ?").bind(p.projectId, userId).first();
    ...
    const batch = [
        db.prepare("UPDATE projects SET data = ? WHERE id = ? AND user_id = ?").bind(JSON.stringify(pData), p.projectId, userId)
    ];

    if (p.printerId && p.printerId !== 'none') {
        batch.push(db.prepare("UPDATE printers SET horas_totais = horas_totais + ?, status = 'printing' WHERE id = ? AND user_id = ?").bind(toNum(p.totalTime), p.printerId, userId));
    }

    if (Array.isArray(p.filaments)) {
        p.filaments.forEach(f => {
            if (f.id && f.id !== 'manual') {
                batch.push(db.prepare("UPDATE filaments SET peso_atual = MAX(0, peso_atual - ?) WHERE id = ? AND user_id = ?").bind(toNum(f.peso || f.weight), f.id, userId));
            }
        });
    }
