...
export async function handleSettings(method, db, userId, request) {
    ...
    if (['POST', 'PUT'].includes(method)) {
        ...
        await db.prepare(`
            INSERT INTO calculator_settings (user_id, custo_kwh, valor_hora_humana, custo_hora_maquina, taxa_setup, consumo_impressora_kw, margem_lucro, imposto, taxa_falha, desconto, whatsapp_template) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) 
