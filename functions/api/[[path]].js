...
export async function onRequest(context) {
    ...
    try {
        ...
        // 3. MANUTENÇÃO DO SCHEMA (Garante que as tabelas existam)
        await db.batch([
        ...
        switch (entity) {
            ...
            case 'impressoras':
                ...
                if (['POST', 'PUT'].includes(method)) {
                    ...
                    await db.prepare(`INSERT INTO printers (id, user_id, nome, marca, modelo, status, potencia, preco, rendimento_total, horas_totais, ultima_manutencao_hora, intervalo_manutencao, historico) 
                        ...
                        .bind(id, userId, p.nome || p.name, p.marca || "", p.modelo || "", p.status || 'idle', toNum(p.potencia), toNum(p.preco), toNum(p.rendimento_total), toNum(p.horas_totais), toNum(p.ultima_manutencao_hora), toNum(p.intervalo_manutencao, 300), JSON.stringify(p.historico || [])).run();
            ...
            case 'projects':
                if (method === 'GET') {
                    ...
                    return sendJSON((results || []).map(r => ({ ...r, data: JSON.parse(r.data || "{}") })));
                }
                if (['POST', 'PUT'].includes(method)) {
                    const p = await request.json();
                    const id = String(p.id || idFromPath || crypto.randomUUID());
                    const label = String(p.label || p.entradas?.nomeProjeto || "Novo Orçamento");
                    const dataStr = JSON.stringify({ entradas: p.entradas || {}, resultados: p.resultados || {}, status: p.status || 'rascunho' });
                    await db.prepare("INSERT INTO projects (id, user_id, label, data) VALUES (?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET label=excluded.label, data=excluded.data").bind(id, userId, label, dataStr).run();
                    return sendJSON({ id, label });
                }
                if (method === 'DELETE') {
                    const id = url.searchParams.get('id');
                    id ? await db.prepare("DELETE FROM projects WHERE id = ? AND user_id = ?").bind(id, userId).run() : await db.prepare("DELETE FROM projects WHERE user_id = ?").bind(userId).run();
                    return sendJSON({ success: true });
                }
                break;
